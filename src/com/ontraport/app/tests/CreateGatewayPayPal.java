...
public class CreateGatewayPayPal extends AbstractTest
{
    @Test
    public void testCreateGatewayPayPal ()
    {
        ...
        gateway_ListAll.formSearch.find(value.get("Sales", "gateway_paypal"));
        
        if(gateway_ListAll.verifyGatewayName(value.get("Sales", "gateway_paypal"))==null)
        {
            fail("couldnt find gateway");
        }
        Gateway_Edit gateway_Edit = gateway_ListAll.clickGateway(value.get("Sales", "gateway_paypal"));

        if(gateway_Edit.verifyGatewayName(value.get("Sales", "gateway_paypal"))==null)
        {
            fail("couldnt find gateway name");
        }
        if(gateway_Edit.verifyLogin(value.get("Sales", "loginemail"))==null)
        {
