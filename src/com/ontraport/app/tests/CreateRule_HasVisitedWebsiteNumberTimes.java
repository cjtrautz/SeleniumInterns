...
public class CreateRule_HasVisitedWebsiteNumberTimes extends AbstractTest
{
    @Test
    public void testCreateRule_HasVisitedWebsiteNumberTimes ()
    {
        Contact_ListAll contactListAll = (Contact_ListAll) new Contact_ListAll().init();
        
        Rule_ListAll rule_ListAll = contactListAll.menuPrimary.clickRuleListAll();
        Rule_Create rule_Create = rule_ListAll.clickNewRule();
        rule_Create.enterRuleName("SelRuleHasVisitedWebsiteNumberTimes");
        rule_Create.openWhenTriggerPane(1);
        rule_Create.selectWhenDrillDownPaneOption("Contact is created");
        rule_Create.openIfConditionPane(1);
        rule_Create.selectIfDrillDownPaneOption("Has visited a website a certain number of times");
        rule_Create.openIfConditionPane(1);
        rule_Create.selectIfDrillDownPaneOption("Any Website");
        ...
        if(rule_ListAll.verifyRule("SelRuleHasVisitedWebsiteNumberTimes")==null)
        {
            fail("couldnt find created rule");
        }
        Rule_Edit rule_Edit = rule_ListAll.clickRule("SelRuleHasVisitedWebsiteNumberTimes");
        if(rule_Edit.verifyText("Has visited ")==null)
        {
            fail("didnt select has visited website number of times");
        }
        if(rule_Edit.verifyIfDropDownInputText(1, "Any Website")==null)
        {
