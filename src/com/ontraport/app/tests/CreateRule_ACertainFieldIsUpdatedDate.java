...
public class CreateRule_ACertainFieldIsUpdatedDate extends AbstractTest
{
    @Test
    public void testCreateRule_ACertainFieldIsUpdatedCheckBox ()
    {
        ...
        rule_Create.openWhenTriggerPane(1);
        rule_Create.enterWhenInput("date", 1);
        ...
        rule_ListAll.formSearch.find(value.get("Rules", "when_date_is_updated"));
        
        //verify that rule exists
        if(rule_ListAll.verifyRule(value.get("Rules", "when_date_is_updated"))==null)
        {
            fail("couldnt find created rule");
        }
        Rule_Edit rule_Edit = rule_ListAll.clickRule(value.get("Rules", "when_date_is_updated"));
        if(rule_Edit.verifyText(" is Updated")==null)
        {
            fail("didnt select field updated");
        }
        if(rule_Edit.verifyWhenDropDownInputText(1, value.get("Fields", "Date_Field"))==null)
        {
