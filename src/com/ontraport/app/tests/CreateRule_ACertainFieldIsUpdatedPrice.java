...
public class CreateRule_ACertainFieldIsUpdatedPrice extends AbstractTest
{
    @Test
    public void testCreateRule_ACertainFieldIsUpdatedPrice ()
    {
        Contact_ListAll contactListAll = (Contact_ListAll) new Contact_ListAll().init();
        
        Rule_ListAll rule_ListAll = contactListAll.menuPrimary.clickRuleListAll();
        Rule_Create rule_Create = rule_ListAll.clickNewRule();
        rule_Create.enterRuleName(value.get("Rules", "when_price_is_updated"));
        ...
        rule_ListAll.formSearch.find(value.get("Rules", "when_price_is_updated"));
        
        //verify that rule exists
        if(rule_ListAll.verifyRule(value.get("Rules", "when_price_is_updated"))==null)
        {
            fail("couldnt find created rule");
        }
        Rule_Edit rule_Edit = rule_ListAll.clickRule(value.get("Rules", "when_price_is_updated"));
        if(rule_Edit.verifyText(" is Updated")==null)
        {
            fail("didnt select field updated");
        }
        if(rule_Edit.verifyWhenDropDownInputText(1, value.get("Fields", "Price_Field"))==null)
        {
