...
public class CreateRule_GetsRefundOnProduct  extends OntraportFirefoxTest {

	AppUtilities appUtilities = new AppUtilities();

	@Test
	public void testCreateRule_GetsRefundOnProduct() throws Exception {
		driver.get(baseUrl + "/");
		//login

		appUtilities.loginToApp(driver, "tester", "passphrases are easy to break");
		String ruleName = "SelRPrdtPur"+Calendar.getInstance().getTimeInMillis();
		...
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select Trigger...", "Gets a refund on a Product");
		Thread.sleep(2000);
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select Product", "Any Product");
		Thread.sleep(2000);
		...
		//Assert Rule Name in the text box of rules details page
		Assert.assertEquals("RuleNameAssertion",ruleName, driver.findElement(By.xpath("//input[@type='text']")).getAttribute("value"));
		
		Assert.assertEquals("Rule When it happens assertion","Gets a refund on", driver.findElement(By.cssSelector("span.sem-statement-text-wrapper")).getText().trim());
		Assert.assertEquals("Product type Assertion","Any Product", driver.findElement(By.xpath("(//input[@type='text'])[2]")).getAttribute("value").trim());
