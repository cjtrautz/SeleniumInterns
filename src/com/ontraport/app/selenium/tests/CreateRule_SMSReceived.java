...
public class CreateRule_SMSReceived extends OntraportFirefoxTest {

	AppUtilities appUtilities = new AppUtilities();

	@Test
	public void testCreateRule_SMSReceived() throws Exception {
		driver.get(baseUrl + "/");
		//login

		appUtilities.loginToApp(driver, "tester", "passphrases are easy to break");
		String ruleName = "SelSMSRec"+Calendar.getInstance().getTimeInMillis();
		...
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select Trigger...", "SMS Received from contact");
		Thread.sleep(2000);
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select Number", "Any Number");
		Thread.sleep(5000);
		...
		Assert.assertEquals("Rule When it happens assertion","SMS Received from contact on", driver.findElement(By.cssSelector("span.sem-statement-text-wrapper")).getText().trim());
		Assert.assertEquals("Sequence type Assertion","Any Number", driver.findElement(By.xpath("(//input[@type='text'])[2]")).getAttribute("value").trim());
