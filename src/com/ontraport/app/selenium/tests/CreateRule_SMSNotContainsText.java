...
public class CreateRule_SMSNotContainsText extends OntraportFirefoxTest{

	AppUtilities appUtilities = new AppUtilities();

	@Test
	public void testCreateRule_SMSContainsEmailAddr() throws Exception {
		driver.get(baseUrl + "/");
		//login

		appUtilities.loginToApp(driver, "tester", "passphrases are easy to break");
		String ruleName = "SMSNoText"+Calendar.getInstance().getTimeInMillis();
		...
		appUtilities.selectRuleDropDown(driver, "IF THIS IS TRUE:", "Select Condition (optional)...", "SMS Does Not Contain Text");
		Thread.sleep(3000);
		...
		Assert.assertEquals("RuleNameAssertion",ruleName, driver.findElement(By.xpath("//input[@type='text']")).getAttribute("value").trim());
		Assert.assertEquals("SMS Does Not Contain", driver.findElement(By.xpath("//div[@id='ussr-chrome-panel-pane']/div[4]/div/div/div/div/div[2]/div[2]/div/div/div/div[2]/div[2]/div/span")).getText().trim());
		...
		Assert.assertEquals("test", driver.findElement(By.xpath("(//input[@type='text'])[3]")).getAttribute("value").trim());
