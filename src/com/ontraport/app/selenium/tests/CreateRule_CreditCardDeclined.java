...
public class CreateRule_CreditCardDeclined extends OntraportFirefoxTest {

	AppUtilities appUtilities = new AppUtilities();

	@Test
	public void testCreateRule_CreditCardCharged() throws Exception {
		//driver.get(baseUrl + "/");
		//login
		WebDriver driver;
		try {
		...
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select Trigger...", "Credit card is charged or declined");
		Thread.sleep(2000);
		
		appUtilities.selectRuleDropDown (driver, "WHEN THIS HAPPENS:","Select...", "Declined");
		Thread.sleep(5000);
		...
		Assert.assertEquals("Rule When it happens assertion","Credit card is", driver.findElement(By.cssSelector("span.sem-statement-text-wrapper")).getText().trim());
		Assert.assertEquals("Assertion","Declined", driver.findElement(By.xpath("(//input[@type='text'])[2]")).getAttribute("value").trim());
